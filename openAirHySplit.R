...
read.files<-function(hours = 96, hy.path, ID) 
{
  ...
  #print(files)
	...
	traj <- read.table(paste0(hy.path, "working/", combine.file.name, sep=""), header = FALSE)
	...
	traj$date2 <- with(traj, ISOdatetime(year, month, day, hour, min = 0, sec = 0, tz = "EST")) # Setup the Canada, ON timezone
...
add.met <- function(month, Year, met, bat.file, control.file) 
{
	...
	## add first line
	write.table(paste("echo", met, ">>", control.file, sep=" "), bat.file, col.names = FALSE, row.names = FALSE, quote = FALSE, append = TRUE)
...
# The procTraj function is designed for parallel execution
procTraj <- function(lat = 51.5, lon = -0.1, year = 2010, start.month=05, start.day=01, end.month=05, 
                     end.day=02, hour.interval="1 hour", name = "london",
                     met = "c:/users/david/TrajData/", out = "c:/users/david/TrajProc/", 
                     hours = 12, height = 100, hy.path = "/home/thalles/Desktop/hysplit/trunk/", ID) 
{
    ...
    bat.file <- paste0(hy.path, "working/", bat.file.name) ## name of BAT file to add to/run
    ...
    for (i in 1:length(dates)) 
	{
        ...
        # format: [1-9]+_[1-9]+...
        # the first number represents the interation ID, it is necessary to insure that each
        # thread will create a separate group of CONTROl.files
        # the second number represents the iD of the trajectory, which can be determined
        # by the number of trajectories for each individual point
		...
		write.table(shbang, bat.file, col.names = FALSE, row.names = FALSE, quote = FALSE)
        ...
        for (i in 1:3)
			add.met(months[i], Year, met, bat.file, control.file)

		x <- paste("echo ./ >>", control.file, sep=" ")
		write.table(x, bat.file, col.names = FALSE, row.names = FALSE, quote = FALSE, append = TRUE)

		x <- paste("echo tdump", "_", ID, "_", year, month, day, hour, " >> ", control.file, sep = "")
		write.table(x, bat.file, col.names = FALSE, row.names = FALSE, quote = FALSE, append = TRUE)

        x <- paste("/home/thalles/Desktop/hysplit/trunk/exec/hyts_std", control.file.extension, sep=" ")
		write.table(x, bat.file, col.names = FALSE, row.names = FALSE, quote = FALSE, append = TRUE)
	...
	traj <- read.files(hours, hy.path, ID)
...
#     files <- list.files(path = path.files, pattern = paste("tdump_", ID))
#     lapply(files, function(x) file.remove(x))
#     
#     # remove existing CONTROL. and MESSAGE files
#     files <- list.files(path = path.files, pattern = paste("CONTROL.", ID))
#     lapply(files, function(x) file.remove(x))
#     
#     files <- list.files(path = path.files, pattern = paste("MESSAGE.", ID))
#     lapply(files, function(x) file.remove(x))
#     
#     files <- list.files(path = path.files, pattern = paste("Rcombined_", ID))
#     lapply(files, function(x) file.remove(x))
#     
#     # Delete all the script files
#     files <- list.files(path = path.files, pattern = paste(script.name, "_", ID, sep=""))
#     lapply(files, function(x) file.remove(x))
...
cleanWD<-function(hy.path = "/home/thalles/Desktop/hysplit/trunk/")
{
    ...
    # Delete all the script files
    files <- list.files(path = path.files, pattern = paste(script.name, "_", sep=""))
    lapply(files, function(x) file.remove(x))
