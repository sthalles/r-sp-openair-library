  ...
  function(lat = 51.5, lon = -45.1,
    ...
    if (add.new.column == T){
      if( !missing(new.column.name) & !missing(new.column.value) ){
        ...
        # create a matrix with values in [new.column.value]
        # this matrix is created byrow which means that if [new.column.value] is
        # a vector, then each value in the vector represents a column in the
        # output data frame
        m <- matrix( new.column.value, # the data elements 
                     nrow = 1,
        ...
        # if [new.column.name] is a vector, each element in the vector
        # represents one column name of the output data frame
        names(df) <- new.column.name
